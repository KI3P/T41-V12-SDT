...
//Serial.print("SAM_carrier_freq_offset= ");Serial.println(SAM_carrier_freq_offset);
//Serial.print("SAM_carrier_freq_offsetOld= ");Serial.println(SAM_carrier_freq_offsetOld);

  //if (SAM_carrier_freq_offset != SAM_carrier_freq_offsetOld) {
    tft.setCursor(OPERATION_STATS_X + 200, FREQUENCY_Y + 30);
    tft.fillRect(OPERATION_STATS_X + 200, FREQUENCY_Y + 30, tft.getFontWidth() * 8, tft.getFontHeight(), RA8875_BLUE);
    tft.print(0.20000012146 * SAM_carrier_freq_offset, 3);  //AFP 11-01-22
 // }
 SAM_carrier_freq_offsetOld=SAM_carrier_freq_offset;
 /* SAM_carrier_freq_offsetHz=0.20024 * SAM_carrier_freq_offset;
  if (millis() - SamTimer >= 60000) {
    SAM_carrier_freq_offsetHz = 0.95 * SAM_carrier_freq_offsetHzOld + 0.05 * 0.20024 * SAM_carrier_freq_offsetHz;
    //Serial.print("Frequency offset (Hz)=  ");
    //Serial.println(SAM_carrier_freq_offsetHz, 4);
    SamTimer = millis();
  }*/

  //SAM_carrier_freq_offsetHzOld = SAM_carrier_freq_offset;
}
